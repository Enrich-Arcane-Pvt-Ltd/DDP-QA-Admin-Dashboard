...
interface ModalProps {
    id: number;
    analysts: AssignQAAnalysts;
    onCancel?: () => void;
    isSubmitting: boolean;
    onSubmit: (qaAnalystId: number) => Promise<boolean>;
}

function AssignQA({ id, analysts, onCancel, isSubmitting, onSubmit} : ModalProps) {
    const [analyst, setAnalyst] = useState(analysts.selectedQaAnalyst ?? "");    
            ...
            <div onClick={(e) => e.stopPropagation()} className="bg-gradient-to-br from-primary-100 to-primary-400 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 animate-slideUp">
                <div className="relative bg-gradient-to-r from-primary-700 to-primary-600 rounded-t-2xl p-6 overflow-hidden">
                    ...
                    <div className="relative flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold text-white">Assign Design QA</h2>
                                <p className="text-primary-100 text-sm">Assign a QA to Design Product</p>
                            </div>
                        </div>
                        <button
                            onClick={onCancel}
                ...
                <div className="p-6 space-y-5">
                    <div className="space-y-2">
                        ...
                        <CustomSelect
                            ...
                            options={(analysts.qaAnalysts ?? []).map(a => ({ value: a.value.toString(), label: a.label }))}
                            icon={<User />}
                ...
                <div className="p-6 pt-0 flex flex-col sm:flex-row gap-3">
                    ...
                    <button
                        ...
                        onClick={async () => {
                            if (!analyst) {
                                toast.error("Please select a QA Analyst");
                                return;
                            }

                            const success = await onSubmit(Number(analyst));
                            if (success) {
                                onCancel?.();
                            }
                        }}
