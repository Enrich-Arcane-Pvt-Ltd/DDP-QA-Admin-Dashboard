...
import { Shield, X, Package, FileText } from "lucide-react";

import { ProductTypeStatus, ProductType } from "@/app/types/Products";
import CustomSelect from "@/app/components/CustomSelect";
import { toast } from "@/app/components/ToastContainer";

interface ModalProps {
    onSubmit?: (data: ProductType) => Promise<boolean>;
    onCancel?: () => void;
    data?: ProductTypeStatus[];
    isSubmitting: boolean;
}

function CreateProductType({ onSubmit, onCancel, data, isSubmitting} : ModalProps) {
    ...
    const handleClick = async () => {
        ...
        const success = await onSubmit?.({ type_name: name, status, description });
        if (success) onCancel?.();
    };
            ...
            <div onClick={(e) => e.stopPropagation()} className="bg-gradient-to-br from-primary-100 to-primary-400 rounded-2xl shadow-2xl w-full max-w-lg transform transition-all duration-300 animate-slideUp">
                <div className="relative bg-gradient-to-r from-primary-700 to-primary-600 rounded-t-2xl p-6 overflow-hidden">
                    ...
                    <div className="relative flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold text-white">Create Product Type</h2>
                                <p className="text-primary-100 text-sm">Add a new product type</p>
                            </div>
                        </div>
                        <button
                            onClick={onCancel}
                ...
                <div className="p-6 space-y-5">
                    <div className="space-y-2">
                        <label className="flex items-center gap-2 text-sm font-semibold text-primary-800">
                            ...
                            Product Type Name <span className="text-error-600">*</span>
                        </label>
                        <CustomInput 
                            type='text'
                ...
                <div className="p-6 pt-0 flex flex-col sm:flex-row gap-3">
                    ...
                    <button
                        ...
                        onClick={handleClick}
                    >
                        {isSubmitting ? 'Creating Product Type...' : 'Create Product Type'}
                    </button>
                </div>
            </div>
        </div>
