...
import { FileText, Shield, User, X } from "lucide-react";
import { toast } from "@/app/components/ToastContainer";

import { ProductTypes, ProductTypeStatus, ProductType } from "@/app/types/Products";
import CustomTextArea from "@/app/components/CustomTextArea";

interface ModalProps {
    row: ProductTypes;
    onSubmit?: (data: ProductType) => Promise<boolean | undefined>;
    onCancel?: () => void;
...
function EditProductType({ onSubmit, onCancel, row, data, isSubmitting } : ModalProps) {
    const [id, setId] = useState(row.id);
    const [name, setName] = useState(row.type_name ?? "");
                ...
                <div className="relative bg-gradient-to-r from-primary-700 to-primary-600 rounded-t-2xl p-6 overflow-hidden">
                    ...
                    <div className="relative flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold text-white">Edit Product Type</h2>
                                <p className="text-primary-100 text-sm">Edit the product type</p>
                            </div>
                        </div>
                        <button
                            onClick={onCancel}
